...
read_fasta2.py  -i  $FASTA  -o  $DIR/$FASTA_FILE  >  $DIR/$FASTA_FILE/organelle.out  2>  $DIR/$FASTA_FILE/organelle.err
...
MODEL=/global/projectb/scratch/andreopo/GAA-1330_fungal/weka-3-6-12/MODEL/mainfungal_mito_jgi_releases_ALLFEATURESmanymers_BALANCED.NB.model
...
for i in `find $DIR/$FASTA_FILE -name features.txt-NEW.arff.NB.pred.MITOLINES.MITOCONTIGS` ; do iDIR=$(dirname $i); filter_fasta.pl -k $i $iDIR/contigs.fa > $i.fa ; done
